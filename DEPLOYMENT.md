...
| L2 | 可信 | L1 + 两份样本过 §3 的**绝对分子地板**（`酒盒.dwg closed_total ≥ 7`、`圆盘盒.dwg closed_total ≥ 32` 且 `role_known_total ≥ 8`）+ 角色可识别 | 成立（本机实测 2026-09-23：`closed_total` 134/263、255/312；`role_known_total` 圆盘盒 9 件 —— 旧分母 64 时是 8 件） |
...
跑完整条八步 flow → 读零件文档/单件详情/挤出，跑完删掉临时目录，并核对**两个根**的
`meta.json` 数量前后不变：真正在用的运行目录 `tech_app/tech_data`（`tech_app_launch.py` 的 `DATA_DIR`
缺省值，34 上 60 个业务项目全在这儿）与历史/杂项目录 `tech_app/data`（34 上 0 个业务项目）。
**只盯后者等于没证明什么**（恒等于 `0 → 0`），而且运行目录里当前 0 个项目时脚本会显式打
「这条断言在新机器上证明不了什么」。两步互补：第 6 步证明「**某个真实项目的数据**是对的」，
第 6b 步证明「**这台机器的链路**是通的」；第 6b 步每次部署都会跑，不过就非零退出。

```bash
# 项目 id 从哪儿来：跑过"一键解析图纸"的项目才有零件文档，下列命令直接把 id 打出来
# 注意根目录是 tech_app/tech_data（运行目录）；tech_app/data 只是历史/杂项目录（34 上 0 个项目）
ls -1 tech_app/tech_data/*/packaging_parts.json 2>/dev/null | cut -d/ -f3
