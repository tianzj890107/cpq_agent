...
- 链路：`git fetch --prune gitlab 20260909` → `git merge --ff-only FETCH_HEAD` → 归档 `nohup.out` →
  **先停 8012 子进程再停 8010 父进程** → 轮询端口释放 → 带 `CPQ_ENV_FILE=/home/wugefei/CPQ/cpq_env.sh`
  用原命令行 `setsid nohup ./open-claude/.venv/bin/python cpq_suite_server.py --host 0.0.0.0 --port 8010` 重启。
- 结果（远端原始输出要点）：
  · 部署前服务在 `HEAD=254c231`（fetch 输出 `254c231..016b1d5`）；纯快进 16 文件，含 2 个新增文件；
...
## 142. 过程行「运行中圆圈 / 真折叠 / 状态展示行用 ·」（9-18，Codex 只改 Spec + 红测）

用户现场（技术工艺智能体 3.2 参数推荐 / 3.3 组装工艺）反馈三处仍未收口：①「正在的时候应该是
圆圈，但是实际上正在的时候就已经是 ✓ 了」；②「里面还是有多余的很奇怪的缩进和换行，而且这些
并没有折叠展开而是都还是显示出来」；③ 点名 `产品族 …已给出（必填 4/11）`、`报价必填项仍缺：…`
这类「展示状态」的行应该用 `·` 而不是 ✓。

- Spec：新增 `docs/specs/process-row-running-info-and-fold.md`（## 133 / ## 136 的增补）。
- 红测：新增 `tests/test_process_row_running_info_and_fold_red.py`（26 条）。用户另有硬要求
  「不只是图纸解析的输出气泡，而是报价和技术工艺所有的都应该是统一的」，因此除技术工艺左栏
  （A/B/C/D 组）外，E 组把同一合同压到另外三个过程行入口：3 阶段页
  `assembly-integration.js::aiProcessCard`、4 阶段页 `cost-review.js::crCard`、报价页
  `确认需求解析结果.html::addToolActivity`。
- 已查实根因（`agent-chat.js::pushTaskStep`，node 实跑真渲染器，非推断）：
  · `:1658-1659` 没有显式状态的行默认 `completed` → 卡片在跑时也渲染 ✓；
  · `:1688-1695` `opensCall` 分支把先到的普通行 `item.append(row)` 裸挂成调用行的直接子节点，
    没有折叠区 / 开关 / 收起态 → 用户看到的「多余缩进 / 换行 + 没折叠 + 全显示」；
  · 统计 / 覆盖率 / 汇总陈述行没有独立行类型，只能跟着步骤行翻 ✓，没有 `·` 的落点。
  · 实测 S1：`icons = ['○','✓','✓','✓','✓','✓','✓']`、`bare=2`。
- 另外三个入口的实测缺口（E 组）：`aiProcessCard` / `crCard` 的 `rowFor()` 把每行硬编码成
  `completed` + ✓（卡片还在跑就全是 ✓，`done()` 的收尾翻转成了死代码）；报价页 `makeItem()`
  运行中是 ○（正确），但 `markTraceDone()` 把状态展示行也翻成 ✓；三个入口都没有 `·` 行类型。
  三处子行折叠、`bare==0`、`details==toggles` 属既有正确能力，E5 作守卫。
- 红测实测：`Ran 26 tests / FAILED (failures=14)`；14 条全部落在本批需求
  （技术侧 A1×4、B1、C1、C2；阶段页 E1×2、E3×4；报价页 E4），其余 12 条为
