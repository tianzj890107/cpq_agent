...
## 426. 落地 `packaging-cost-content-binding-panel`：成本面板说得出「这一单用哪几项包材认不出来」，并被降级披露的包材项逐个点名（22 OK，红基 19 红）（9-22，Codex 实现）

- 唯一 Spec：`docs/specs/packaging-cost-content-binding-panel.md`，红测 `tests/test_packaging_cost_content_binding_panel_red.py`（22 条，实现前 19 红 / 3 绿护栏）。
- 缺口是「同形」：后端 `content_binding`（`compute_project()` 结果体与读侧 `_content_binding_of()`；`source` / `bound_total` / `unbound_total` / `bound_codes` / `unbound_codes`）+ 就绪门 `content_binding_source` 早已在接口上，而前端 `content_binding` / `unbound_codes` / `bound_total` **0 处引用** —— 页面只看到 `bound_gaps = []` 的样子，「这一单一项都不缺」与「我们认不出这一单用了哪几项包材，所以包材缺口只披露不阻断」长得一模一样，§2.4 要求指名道姓的 `unbound_codes` 在界面上不存在。
- 落点：`tech_app/frontend/requirement-confirm.js` 一个文件 —— 新增三个纯函数 `pcContentBinding()`（来源三态 `none` / `authoritative` / `unknown`，闭集外的来源不假装权威也不假装缺失）、`pcContentBindingCodesText()`（`被降级披露的包材项：A、B`，没有码就空串）、`pcContentBindingBlock()`（渲 `data-pc-content-binding="<state>"` + 人话 + `绑定 N · 未绑 M`，有码时加 `data-pc-content-binding-codes="<条数>"`）；`pcPanel()` 在裁决条之后、合计之前插入**一处无条件**调用（不看 `built` / `gaps`）。
- 冻结面：来源只读 `cost.content_binding.source`（不按 `unbound_codes` 是否为空反推、不读 `readiness` / `gaps`），条数取后端给的值（不按码数重算），`unknown` 不许归到 `none` / `authoritative` 任一档，`unbound_codes` 不准混进阻断 / 提示两组缺口；未改后端、未加接口 / 依赖、未改 `index.html`。
- 复跑：`tests.test_packaging_cost_content_binding_panel_red` 22 OK（`node` 真跑渲染串，不只 grep）；Spec §5 不回归 8 份（`cost_content_binding_source_disclosure` / `cost_readiness_panel` / `cost_readiness_severity_layering` / `cost_gaps_scoped_to_order_contents` / `cost_engine` / `cost_gaps` / `cost_red_closure` / `bom_disclosure_panel`）`Ran 183 ... OK`；`node --check tech_app/frontend/requirement-confirm.js` 退出码 0；`test_spec_status_truth_red` 7 OK；`git diff --check` 干净。

## 427. 落地 `packaging-material-map-account-panel`：BOM 面板说全「材料码映射表」的账——空表不再静默、来源与指纹可见、逐档条数不只给命中数（25 OK，红基 21 红）（9-22，Codex 实现）

- 唯一 Spec：`docs/specs/packaging-material-map-account-panel.md`，红测 `tests/test_packaging_material_map_account_panel_red.py`（25 条，实现前 21 红 / 4 绿护栏）。
- 缺口：`business_material_rows` 里 `map_hit_total` / `map_source` / `map_fingerprint` / `map_unavailable` / `reason_counts` 早已在接口上，而前端只用了前两者里的两个信号（`pbMaterialMapNote()` 只在"读不到"或"命中 > 0"时有话说）——**映射表读得到但一条映射都没有**（合法的一版空表）时页面一个字都不说，"空表"与"表里没有这一条"同形；这一版用的是哪一份映射表（仓库内置 vs 环境变量覆盖）与逐档条数（`map_source` / `map_fingerprint` / `reason_counts`）前端 0 处引用。
- 落点（两个文件）：`packaging_bom._business_material_scope()` 新增**一个**键 `map_entry_total`（`len(map_entries or ()) if map_available else 0`，键总是存在、值总是 int；既有九键与五档闭集一字未动）；`requirement-confirm.js` 新增三个纯函数 `pbMaterialMapState()`（状态闭集 `unavailable` / `empty` / `ready` / `unknown`；**缺 `map_entry_total` 的老载荷算未知档，不许当成 0 条**；`hits` 只进 headline、不参与判定）、`pbMaterialMapBreakdown()`（按后端闭集固定顺序给人话，只放非零档，闭集外的档名不猜、合并成「其它档 N 行」）、`pbMaterialMapAccountBlock()`（`data-pb-material-map-state` / `-source` / `-fingerprint` / `-reasons`，只有 `ready` / `empty` 渲染——`unavailable` 由既有那句承担、`unknown` 不编话），`pbPanel()` 在 `${mapBlock}` 之后加**一处**调用。
- 冻结面：状态只读 `map_unavailable` + `map_entry_total`（不按 `reason_counts` / `map_hit_total` 重算表是不是空的）、不改检索口径 / 映射表 JSON / `map_facts()` 键 / `index.html`、未加接口与依赖；既有 `data-pb-material-map="1"` 块与逐行缺口块一字不动。
- 实现期两处收敛（未放宽任何断言）：① `map_entry_total` 是空串时按未知档处理（空串不是"0 条"的事实）；② 红测 `F2` 原先是源码字符串守卫（不允许 `pbMaterialMapState()` 体内出现 `map_hit_total`），与 §C2 要求它同时回 `hits` 自相矛盾 —— 已改成**行为**守卫（`{map_entry_total: 0, map_hit_total: 99}` 必须判 `empty`、`{map_entry_total: 5, map_hit_total: 0}` 必须判 `ready`）。
- 复跑：Spec §5 不回归 6 份（`material_code_map` / `material_unresolved_panel` / `bom_business_material_rows` / `bom_disclosure_panel` / `parametric_bom` / `cost_engine`）`Ran 239 ... OK`；packaging 全域 `discover -s tests -p 'test_packaging_*.py'` `Ran 2312 ... FAILED (failures=5, skipped=8)` —— 仍是那 5 条既有挂账（`part_role_mapping_reaches_card::A2`、`bom_part_size_provenance::B3`、`parse_to_downstream_seams::B4`、`quote_send_recovery::C1`、`route_bom_version_pinning::F2`），本批未引入新红；`node --check tech_app/frontend/requirement-confirm.js` 退出码 0；`test_spec_status_truth_red` 7 OK；`git diff --check` 干净。

## 428. 落地 `packaging-cost-business-parts-read-failure-panel`：成本面板说得出「读不到业务部件清单」——它与「清单没换版」不再同形（17 OK，红基 13 红）（9-22，Codex 实现）

- 唯一 Spec：`docs/specs/packaging-cost-business-parts-read-failure-panel.md`，红测 `tests/test_packaging_cost_business_parts_read_failure_panel_red.py`（17 条，实现前 13 红 / 4 绿护栏）。
- 缺口：`packaging_cost.load_cost()` 的两条分支都带 `business_parts_unavailable`（正常给 `{}`、读挂给 `{code, reason}`），而前端 **0 处引用** —— 清单读不到时面板上只看得到 `stale=false` / `stale_reasons=[]`，与"清单没换版"一模一样；BOM 那条轴（`data-pc-bom-unavailable`）与路线那条轴（`data-pc-route-unavailable`）都各自有一条 banner，第三条轴没有落点。
- 落点：`tech_app/frontend/requirement-confirm.js` 一个文件 —— 新增 `pcBusinessPartsUnavailable()`（三态闭集：那一栏不是对象 / 老载荷缺键 → `unknown` 且**不给话**，空对象 → `ok`，非空 → `unavailable`，`code` 缺失兜底 `business_parts_unavailable`）与 `pcBusinessPartsBanner()`（`data-pc-business-parts-unavailable="<code>"` + 那句「暂时读不到业务部件清单…这不代表没换版」，`reason` 非空才接 `（<reason>）`）；`pcPanel()` 在路线那条轴之后、规则快照那条轴之前加**一处**调用。
- 冻结面：判据只读 `record.business_parts_unavailable`（两函数体内不出现 `stale` / `stale_reasons`；**不许**混进 `PC_STALE_REASONS` 那张"变了"的人话表 —— "读不到"不是"变了"）；未改后端（`_business_parts_drift()` 的"比不了就不报漂移"口径一字未动）、未加接口与依赖、未改 `index.html`；既有四条 banner / 裁决条 / 包材绑定条一字不动。
- 复跑：`tests.test_packaging_cost_business_parts_read_failure_panel_red` 17 OK；Spec §5 不回归 7 份（`business_parts_version_pinning` / `cost_route_version_read_failure` / `cost_gaps` / `cost_readiness_panel` / `cost_content_binding_panel` / `cost_engine` / `cost_red_closure`）`Ran 176 ... OK`；`node --check tech_app/frontend/requirement-confirm.js` 退出码 0；`test_spec_status_truth_red` 7 OK；`git diff --check` 干净。

## 429. 落地 `packaging-bom-business-parts-scope-panel`：BOM 面板说得出「这份 BOM 照哪一版业务部件清单配的」（19 OK，红基 16 红）（9-22，Codex 实现）

- 唯一 Spec：`docs/specs/packaging-bom-business-parts-scope-panel.md`，红测 `tests/test_packaging_bom_business_parts_scope_panel_red.py`（19 条，实现前 16 红 / 3 绿护栏）。
- 缺口：读接口早就逐字给了 `business_parts_id` 与 `business_parts`（`available` / `business_part_total` / `bound_total` / `unbound_total` / `business_parts_hash` / `gap`；`_business_parts_scope()` 还把"读不到"`business_parts_document_unavailable` 与"还没导入"`business_parts_missing` 分成两种 gap），而前端 `business_parts_id` / `business_parts_hash` / `business_part_total` **0 处引用** —— BOM 面板 head 只写了「零件文档版本」这一把尺子；"读不到"与"还没导入"两种态在页面上同形（都什么都不显示），件数为 0 的载荷也一样。
- 落点：`tech_app/frontend/requirement-confirm.js` 一个文件 —— 新增 `pbBusinessPartsScope()`（四态 `ready` / `empty` / `unavailable` / `unknown`；件数只读 `record.business_parts`、可用性只读 `available`；`gap.message` 非空时逐字转达、为空才用兜底句）与 `pbBusinessPartsScopeBlock()`（`data-pb-business-parts-state` / `-id` / `-total` / `-gap` + 那句人话，四态都渲）；`pbPanel()` 在材料码映射表那条账之后加**一处**调用。
- 冻结面：不许用几何件数冒充业务件数（体内无 `stats` / `items`），可用性不许按 `business_parts_stale` 反推（体内无 `stale`），`empty` 不许说成 `ready`、`unavailable` 不许说成 `empty`；`hash` 只在 headline 里截前 12 位（与 head 里零件版本同一口径）；未改后端 / 接口 / `index.html` / 依赖。
- 复跑：`tests.test_packaging_bom_business_parts_scope_panel_red` 19 OK；Spec §5 不回归 6 份（`bom_business_parts_rows` / `business_parts_version_pinning` / `material_code_map` / `material_map_account_panel` / `bom_disclosure_panel` / `parametric_bom`）`Ran 173 ... OK`；`node --check tech_app/frontend/requirement-confirm.js` 退出码 0；`test_spec_status_truth_red` 7 OK；`git diff --check` 干净。

## 430. 落地 `packaging-bom-business-rows-account-panel`：BOM 面板说得出「部件组行是权威清单来的，还是几何模板展开的」（21 OK，红基 17 红）（9-22，Codex 实现）

- 唯一 Spec：`docs/specs/packaging-bom-business-rows-account-panel.md`，红测 `tests/test_packaging_bom_business_rows_account_panel_red.py`（21 条，实现前 17 红 / 4 绿护栏）。
- 缺口：读接口早就逐字给了 `business_rows`（`row_total` / `box_part_total` / `optional_part_total` / `needs_input_total` / `keys`，`_business_rows_scope()` `:689`，判据只认行上的 `source == "packaging_business_parts_authority"` 且 `bom_category` 属部件组；`load_bom()` 顶层键 `:1423`），而前端 `business_rows` **0 处引用** —— 材料组那一半已经有映射表与清单版本两块落点，**部件组这一半一行都没有**：权威清单来的 28 行与几何模板展开的 10 行在表格里同形（真样本实测：导入前 `row_total=0`、导入后 28），用户看不出该不该去导权威清单；`needs_input_total`（其中几行还缺输入）也 0 处，合计区的"缺输入"是全部行的，分不出权威清单那部分有几行要补。
