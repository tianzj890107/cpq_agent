...
1. 单件图只按既有图层角色着色，不做"看图增强"（颜色合并 / 过滤 / 图层显隐开关）；
2. 任务文件里点图纸那张整图不做拖拽缩放（Spec `packaging-task-file-dwg-opens-the-whole-plan.md` §3 边界 1）；
3. 未选中态仍是整张平面图（点图选件的入口）；若连它也不要，需另开一批并说明替代入口。
