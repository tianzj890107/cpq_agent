...
### 7.4 `A2` 已按 §7.3 的修法转绿（`## 468`，Codex 测试侧；断言意图不减）

§7.3 记明 `A2` 的 `1.0` 与同文件 `B3` 互斥，并写明修法是「把 `1.0` 改成 `0.5`」。本批按此执行：
`test_a2_role_known_ratio_moves_with_the_mapping` 的期望值 `1.0` → `0.5`（探针两件、映射一件，
`role_known_ratio = 1/2`）。这条断言要证的事没变、也没被绕开：**人工映射必须把这一件从
「未知」抬成「已知」**（`0.0 → 0.5`），反向对照把期望值改回 `1.0` 立即 `FAIL`。

同一文件的 `B3`（配对不上的件不许被改）逐字未动。本层 8 条全绿。
