...
结论：**"酒盒 28 件"这次是查表查出来的，不是从图里推出来的**。

> `## 458` 更正：上表只能证明「**名称直接命中 18 件**」，推不出「识别上限 18 件」。
> `## 458` 之后，件数由**全部 DWG 证据**决定，名称文字只是其中一类证据；本表保留为
> 写入当时（2026-09-23）的只读实测记录。仓库里那份
...
- 件数由**全部图纸证据**决定（文字 + 几何轮廓 + 尺寸 + 引线 + 图块 + 图层 + 空间关系 + 重复/镜像关系），
  不许硬凑成某个数字，也不许从资料里补进来。
  ⚠ 本批原文把"上限"写成了**名称文字**（酒盒实测规范化同名 18 件）——那是**名称直接命中率**，
  不是识别能力上限：图纸里没有"底板"两个字，不等于图上没有底板（可能有闭合轮廓、长宽尺寸、引出线、
  材料注记、与相邻件的空间关系）。这条口径已由 `## 458`
  （`packaging-business-parts-must-come-from-all-drawing-evidence.md`）推翻并收窄。
  本批留下的、仍然管事的只有一句：**缺同名文字 ≠ 图上没有这一件**，件数只能由证据决定；
  也只允许由证据决定（不许用 BOM / 金标凑数）。
...
> `## 458` 收窄：本批实现（`_derived_rows()`：一条名称锚点 = 一件）满足的是 §2.2 的产出契约，
> **不**满足 §1 新口径「件数由全部图纸证据决定」——几何/尺寸/引线/父子分组都还没进「推件」这一步，
> 真样本实测 recall 19/28、只有 5 件带尺寸、22 件 `unbound`。新的验收门槛见
> `docs/specs/packaging-business-parts-must-come-from-all-drawing-evidence.md`。
