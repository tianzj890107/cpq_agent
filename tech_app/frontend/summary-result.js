...
const srPid = new URLSearchParams(location.search).get('project') || localStorage.getItem('cad_engine_project_id') || '';
...
function srWorkflow(){return `<section class="workflow-section"><div class="main-workflow"><div class="main-step-wrapper"><div class="main-step completed"><div class="main-step-number">1</div><span>接受工艺评估需求</span></div></div><div class="main-connector active"></div><div class="main-step-wrapper"><div class="main-step completed"><div class="main-step-number">2</div><span>解析技术工艺过程</span></div></div><div class="main-connector active"></div><div class="main-step-wrapper"><div class="main-step active"><div class="main-step-number">3</div><span>输出工艺评估结果</span></div><div class="sub-labels-row"><span class="sub-label active">3.1 汇总结果</span><span class="sub-label-arrow">→</span><span class="sub-label">3.2 审核报告</span><span class="sub-label-arrow">→</span><span class="sub-label">3.3 发布报告</span></div></div></div></section>`;}
...
// 2.3 成本测算 → 3.1 汇总：金额只做展示格式化，前端不对成本求和（合计由后端 2.3 口径给出）。
...
// 2.3 的确认状态优先取聚合层顶层口径 aggregate.cost（backend services/summary.py 复用
...
// 经济可行性只有三种状态：2.3 已确认 → 可行；已测算未确认 / 没做 → 待评估。
...
// 2.3 阶段行：只有成本已确认（结论完整）才进汇总表 —— 空着也加一行的话，那行必然带
// 「尚未/暂无」，而后端送审门禁禁止 stage_results 出现这两个词。
function srCostStage(cost,review){const info=srCostInfo(cost,review);if(!info.confirmed)return[];const amounts=srCostAmounts(info);return [{stage:'2.3 成本测算',conclusion:`${amounts.length?`${amounts.join('，')}；`:''}${srCostConfirmedText(info)}。`}];}
...
// 2.2 组装与整合：**有结果才进汇总表**。空着也加一行的话，那行结论必然写着「尚未…」，
// 而后端送审门禁(_report_content_issues)恰好禁止 stage_results 出现「尚未/暂无」——
// 没走 2.2 的老项目会因此再也提交不了报告。
function srIntegrationStage(doc){
  ...
  return[{stage:'2.2 组装与整合',conclusion:`${bits.join('，')}。`}];
}
